"""
Cohort Builder (Streamlit)
Author: Mohamed Shez • Created - 14‑May‑2025

This **Build Cohort** page lives under *pages/Build_Cohort.py* in your multi‑page
Streamlit app.  Navigation is already handled by Streamlit’s built‑in sidebar,
so the extra radio‑button router and the other page branches have been removed.
"""
...
class BuildCohort:
    ...
    #         render_header('Build Cohort',
    #                       'First use the Primary Filters on the sidebar to filter down the dataset. The Secondary Filters are then populated for you to build your Cohort.')
    #         finalize_cohort.run()
    #
        ...
        with st.container():
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    ...
                    "FILTER_TYPE": st.column_config.SelectboxColumn("Filter Type", options=["dropdown", "text filter",
...
class FinalizeCohort:
    ...
    def prepare_filter_data_frames(self, filter_type):
        if filter_type == "Primary":
            st.session_state.secondary_filters_df = st.session_state.secondary_filters_df[
                ["COLUMN_NAME", "DATA_TYPE", "FILTER_TYPE"]]
    ...
    def generate_filter_clause(self, filter_type):
        non_text_filters_clause = gen_where_clause_for_non_text_filters(st.session_state.filter_df,
    ...
    def apply_filters(self, filter_type):
        ...
        st.session_state.preview_dataset = read_table(
            st.session_state.final_query.replace('COUNT(*)', '*') + ' LIMIT 100',
            st.session_state.session
        )
            ...
            if filter_type == "dropdown":
                ...
                try:
                    selected_date = pd.to_datetime(
                        st.session_state.selected_filters[column_name].get("value", min_date))
                ...
                if condition == "between":
                    ...
                    with col1:
                        value1 = st.number_input(f"{column_name}_value1", min_value=min_value, max_value=max_value,
                                                 value=value1)
                    with col2:
                        value2 = st.number_input(f"{column_name}_value2", min_value=min_value, max_value=max_value,
                                                 value=value2)
                    st.session_state.selected_filters[column_name] = {"condition": condition, "value1": value1,
                                                                      "value2": value2}
                else:
                    try:
                    ...
                    st.session_state.selected_filters[column_name] = {"condition": condition,
                                                                      "value": st.number_input(f"{column_name}_value",
                                                                                               min_value=min_value,
                                                                                               max_value=max_value,
                                                                                               value=value)}
    ...
    def render_main_content(self):
        ...
        with col1:
            ...
            if st.session_state.is_cohort_saved:
                st.info(
                    "If you scheduled your cohort as a Dynamic table, updating the Cohort will require you to re-create your schedule.")
...
class ScheduleCohorts:
    ...
    def check_existing_table(self):
        registry_db = st.session_state.selected_database
...
class TextFilterDialog:
    def __init__(self, column_name):
        ...
        self.condition_options = ["", "Contains", "Does not contain", "Equals", "Does not equal", "Begins with",
                                  "Ends with", "Blank", "Not blank"]
